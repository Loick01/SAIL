...
let declare_method (llc:llcontext) (llm:llmodule) (name:string) (m:sailor_method) (decls:DeclEnv.t) : DeclEnv.t = 
  let proto =
    ...
    | None -> void_type llc
    in
    let args_type = List.map (fun (_,(_,arg)) -> getLLVMType arg llc llm) m.decl.args |> Array.of_list in
    let method_t = 
      if m.variadic then
        var_arg_function_type llvm_rt args_type
      else
      function_type llvm_rt args_type
     in
    let name' = if String.equal "_Main_" name then "main" else name in

    declare_function name' method_t llm 
