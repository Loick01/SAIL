...
let pp_method (pp_method_body : int -> formatter ->  ('a, tag option) Either.t-> unit) (pf : formatter) (m : 'a method_defn) =
...
let pp_program (pp_method_body : int -> formatter ->  ('a, tag option) Either.t-> unit) (pp_process_body : int -> formatter -> 'a -> unit)  ((pf : formatter) : formatter) (p : 'a sailModule) = 
