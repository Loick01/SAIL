...
let show_context text (p1,p2) =
  ...
  let make ?(why=None) ?(hint=None) ?(label=None) where what : error = 
   ...
   let print_errors (file:string) (errs:errors) : unit =  
    let print_indication fmt file (where: TypesCommon.loc) what : unit = 
    ...
    let s fmt = List.iter (
      fun {where;what;hint;_} ->
        if where = (dummy_pos,dummy_pos) then
          ...
          print_indication fmt file where what;
          match hint with 
          | Some (where,h) -> 
            begin
             match where with
             | Some loc -> print_indication fmt ((fst loc).pos_fname |> open_in |> In_channel.input_all) loc h                
...
module type Logger = sig
  ...
  val iter_warnings : 'a t -> (error list -> unit) -> 'a t
 ...
 module MakeTransformer (M : Monad) : Logger with type 'a t = (('a,error) Result.t * error list) M.t and type 'a old_t = 'a M.t = struct 
  ...
  let iter_warnings (x : 'a t) (f : error list -> unit) : 'a t =
