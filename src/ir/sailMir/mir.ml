...
struct
  ...
  let lower decl _   =
    let rec aux = function
      ...
      | DeclSignal _ -> Error.make TypesCommon.dummy_pos "not_implemented"
      | Skip _ -> buildSingle [] |> ok 
      | Assign (loc, e1, e2) -> 
        buildSingle [AstMir.Assign (loc, e1, e2)] |> ok 
      | Seq (_, s1, s2) ->
        let* s1 = aux s1 and* s2 = aux s2 in buildSeq s1 s2 |> ok
      | Par _ -> Error.make TypesCommon.dummy_pos "not_implemented"
      ...
      | Case _ -> Error.make TypesCommon.dummy_pos "not_implemented"
      | Invoke (_loc, _, id, el) -> buildInvoke id el |> ok
      | Return (_, e) -> buildReturn e |> ok 
      | Run _ -> Error.make TypesCommon.dummy_pos "not_implemented"
      | Emit _ -> Error.make TypesCommon.dummy_pos "not_implemented"
      | Await _ -> Error.make TypesCommon.dummy_pos "not_implemented"
      | When _ -> Error.make TypesCommon.dummy_pos "not_implemented"
      | Watching _ -> Error.make TypesCommon.dummy_pos "not_implemented"
      | Block _ -> Error.make TypesCommon.dummy_pos "not_implemented"
